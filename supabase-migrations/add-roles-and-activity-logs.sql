-- Migration: Add role-based access control and activity logging
-- Date: 2025-12-04
-- Description: Adds role column to admin_users, creates activity_logs table, and inserts new staff users

-- Step 1: Add role column to admin_users table
ALTER TABLE admin_users
ADD COLUMN IF NOT EXISTS role VARCHAR(20) DEFAULT 'staff' CHECK (role IN ('super_admin', 'staff'));

-- Step 2: Update existing DrBBPhetla user to super_admin
UPDATE admin_users
SET role = 'super_admin'
WHERE username = 'DrBBPhetla';

-- Step 3: Insert new staff users (Lerato and Potlako)
-- Note: Use bcrypt to hash these passwords before production
...
-- Step 6: Create a view for easy activity log querying
...
-- This allows authenticated users to read logs, but only super admins can view via application logic
